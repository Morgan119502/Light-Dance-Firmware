...
def start_music():
    ...
    # music_file = r"C:\Users\morga\data\常用\lightdance V3.mp3"  # 替換為你的音樂檔案路徑
...
while code_running:
    ...
    if isRunning == True:
        ...
        if currentTime - rootTime >= 1000*count:
            ...
            # number = currentTime - rootTime + startTime*1000
            # number = int(number)
            mes = color[count%3]
            data = struct.pack("!I", ord(mes))
            current_broadcast_message = mes
            sock.sendto(data, (broadcast_address, port))
            print(f"Broadcasted color:{ord(mes)}")
